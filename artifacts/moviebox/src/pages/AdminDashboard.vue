    ...
    <div class="admin-main">
        ...
        <div class="dash-row">
          <div class="dash-panel">
            ...
            <div class="recent-list">
              <div v-for="m in recentContent.slice(0,6)" :key="m.id" class="recent-item">
          ...
          <div class="dash-panel">
            ...
            <div class="recent-list">
              <div v-for="u in allUsers.slice(0,6)" :key="u.id" class="recent-item">
                <div class="user-av">{{ u.name.slice(0,1).toUpperCase() }}</div>
                <div class="recent-info">
                  <p class="recent-title">{{ u.name }}</p>
                  <p class="recent-meta">{{ u.phone }} · {{ u.role }}</p>
                </div>
              ...
              <tr v-for="m in filteredContent" :key="m.id">
                ...
                <td>
                  <div class="action-btns">
                    <button class="act-btn edit" @click="editContent(m)" title="Edit">✏️</button>
                    <button class="act-btn del" @click="deleteContent(m.id)" title="Delete">🗑️</button>
                  </div>
                </td>
              </tr>
            </tbody>
            ...
            <div class="um-body">
              <div class="um-grid">
                ...
                <div class="form-field" v-if="form.type === 'series' || form.type === 'animation'">
                  <label>Add Episode (for selected series)</label>
                  <div class="ep-add-row">
                    <input v-model="epForm.season" type="number" placeholder="Season" class="ep-input" />
                    <input v-model="epForm.num" type="number" placeholder="Episode #" class="ep-input" />
              ...
              <div class="um-actions">
                ...
                <button class="save-btn" @click="saveContent" :disabled="!form.title || !form.poster">{{ editingId ? 'Update' : 'Upload' }} Content</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-else-if="activeSection === 'users'" class="section-content">
        ...
        <div class="content-table-wrap">
          <table class="content-table">
            <thead>
              <tr><th>Avatar</th><th>Name</th><th>Phone</th><th>Role</th><th>Subscription</th><th>Actions</th></tr>
            </thead>
            <tbody>
              <tr v-for="u in filteredUsers" :key="u.id">
                ...
                <td>
                  <div class="action-btns">
                    <button class="act-btn edit" @click="toggleRole(u)" title="Toggle Role">{{ u.role === 'admin' ? '👤' : '🛡️' }}</button>
                    <button class="act-btn del" @click="banUser(u.id)" title="Ban">🚫</button>
                  </div>
                </td>
              </tr>
            </tbody>
      ...
      <div v-else-if="activeSection === 'activities'" class="section-content">
        ...
        <div class="activity-list">
          <div v-for="(act, i) in activities" :key="i" class="activity-item">
            <div class="act-icon" :class="act.type">{{ act.emoji }}</div>
            <div class="act-info">
      ...
      <div v-else-if="activeSection === 'seo'" class="section-content">
        ...
        <div class="settings-form">
          ...
          <button class="save-btn large" @click="saveSeo">Save SEO Settings</button>
          <div v-if="seoSaved" class="save-success">✅ SEO settings saved successfully!</div>
        </div>
      </div>
      ...
      <div v-else-if="activeSection === 'settings'" class="section-content">
        ...
        <div class="settings-form">
          ...
          <button class="save-btn large" @click="saveSettings">Save Settings</button>
          <div v-if="settingsSaved" class="save-success">✅ Settings saved successfully!</div>
        </div>
      </div>
    </div>
  </div>
...
import { getAllMovies, type Movie } from "../data/movies";
import { useAuth } from "../stores/useAuth";
import { subscriptionPlans } from "../stores/useSubscription";
...
const allMovies = ref<Movie[]>([...getAllMovies()]);
...
const sections = [
  ...
  { id: "content", label: "Content", icon: `<svg width="17" height="17" viewBox="0 0 24 24" fill="none"><rect x="2" y="4" width="20" height="16" rx="2" stroke="currentColor" stroke-width="1.5"/><path d="M8 4v16M16 4v16M2 12h20" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>`, count: 0 },
  { id: "users", label: "Users", icon: `<svg width="17" height="17" viewBox="0 0 24 24" fill="none"><circle cx="9" cy="8" r="3" stroke="currentColor" stroke-width="1.5"/><path d="M2 20c0-4 3.1-7 7-7s7 3 7 7" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><circle cx="17" cy="8" r="2.5" stroke="currentColor" stroke-width="1.5"/><path d="M22 20c0-3-2-5-5-5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>` },
...
const stats = computed(() => [
  { label: "Total Content", value: allMovies.value.length, trend: "+12%", up: true, icon: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><rect x="2" y="4" width="20" height="16" rx="2" stroke="#1cb7ff" stroke-width="1.5"/><path d="M8 4v16M16 4v16" stroke="#1cb7ff" stroke-width="1.5"/></svg>` },
  ...
  { label: "Free Content", value: allMovies.value.filter((m) => m.isFree).length, trend: "+3", up: true, icon: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" stroke="#fbb827" stroke-width="1.5"/></svg>` },
  { label: "Premium Content", value: allMovies.value.filter((m) => !m.isFree).length, trend: "+8%", up: true, icon: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5L12 1z" stroke="#a855f7" stroke-width="1.5"/></svg>` },
]);

const recentContent = computed(() => [...allMovies.value].slice(0, 10));
...
const filteredUsers = computed(() => {
  ...
  if (userSearch.value) users = users.filter((u) => u.name.toLowerCase().includes(userSearch.value.toLowerCase()) || u.phone.includes(userSearch.value));
...
function saveContent() {
  const newItem: Movie = {
    id: editingId.value ?? `custom-${Date.now()}`,
    title: form.title,
    href: `/watch/${editingId.value ?? `custom-${Date.now()}`}`,
    poster: form.poster,
    ...
    rating: form.rating,
    seasons: form.type !== "movie" && form.type !== "live-tv" ? form.seasons : undefined,
    region: form.region,
    isFree: form.isFree,
  };
  if (editingId.value) {
    const idx = allMovies.value.findIndex((m) => m.id === editingId.value);
    if (idx !== -1) allMovies.value[idx] = newItem;
  } else {
    allMovies.value.unshift(newItem);
  }
  closeUpload();
}
...
const activities = ref([
  { emoji: "⬆️", type: "upload", message: "New content uploaded: Mission Impossible Final Reckoning", time: "2 minutes ago" },
  { emoji: "👤", type: "user", message: "New user registered: +256701234567", time: "15 minutes ago" },
  { emoji: "💳", type: "payment", message: "Subscription payment received: UGX 30,000 (1 month)", time: "1 hour ago" },
  { emoji: "⬆️", type: "upload", message: "New content uploaded: Euphoria S3", time: "2 hours ago" },
  { emoji: "🗑️", type: "delete", message: "Content removed: Old Moviebox Archive", time: "3 hours ago" },
  { emoji: "💳", type: "payment", message: "Subscription payment received: UGX 10,000 (1 week)", time: "4 hours ago" },
  { emoji: "👤", type: "user", message: "New user registered: +256772345678", time: "5 hours ago" },
  { emoji: "⚙️", type: "setting", message: "Site settings updated by Admin", time: "1 day ago" },
  { emoji: "💳", type: "payment", message: "Subscription payment received: UGX 5,000 (1 day)", time: "1 day ago" },
  { emoji: "⬆️", type: "upload", message: "New content uploaded: NBS TV Live Channel", time: "2 days ago" },
]);
...
.recent-meta { font-size: 10px; color: rgba(255,255,255,0.4); margin: 2px 0 0; }
